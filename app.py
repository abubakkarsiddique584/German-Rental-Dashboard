...
# app = Flask(__name__)
#
# DATA_PATH = os.path.join(os.path.dirname(__file__), 'data', 'immo_data.csv')
...
#         need_kitchen = request.form.get('kitchen') == 'on'
#         need_balcony = request.form.get('balcony') == 'on'
#
#         # Build the Filter Mask
#         mask = (df['city'] == city) & (df['totalRent'] <= budget) & (df['noRooms'] >= rooms)
#
#         if need_kitchen:
#             mask &= (df['hasKitchen'] == True)
#         if need_balcony:
#             mask &= (df['balcony'] == True)
#
#         filtered_df = df[mask]
#         results_count = len(filtered_df)
#
#         if not filtered_df.empty:
#             recommendation = filtered_df['district'].value_counts().head(5).index.tolist()
#
